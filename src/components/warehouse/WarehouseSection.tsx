      ...
      {!isCollapsed && products.length > 0 && (
        <div className="space-y-1">
          {products.map((product) => (
            <div
              key={product.id}
              className={`bg-white hover:bg-gray-50 border transition-colors cursor-pointer ${
                product.quantity <= 5 ? 'border-l-4 border-l-red-500' : 'rounded-lg'
              }`}
                ...
                <div className="sm:hidden">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                      <div className={`text-xs font-medium ${product.quantity <= 5 ? 'text-red-600' : 'text-gray-900'}`}>
                ...
                <div className="hidden sm:flex items-center">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right min-w-[120px]">
                      <div className="text-sm font-medium text-gray-900">{product.averagePurchasePrice?.toLocaleString()} ₸</div>
                      <div className="text-xs text-emerald-600">{((product.quantity || 0) * (product.averagePurchasePrice || 0)).toLocaleString()} ₸</div>
                    </div>
                    <div className="text-right min-w-[80px]">
                      <div className={`text-sm font-medium ${product.quantity <= 5 ? 'text-red-600' : 'text-gray-900'}`}>
                        {product.quantity} {product.unit}
                      </div>
                    </div>
